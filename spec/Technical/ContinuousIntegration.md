...
[ci.yml](../../.github/workflows/ci.yml) 包含 Check 和 Release 两个任务，使用 Ubuntu 与 Node.js 24。PR、`main` 分支推送及数字开头的标签触发检查；版本标签在检查成功后触发发布。

`package.json` 的 `packageManager` 固定 pnpm 版本，CI 通过 `pnpm/action-setup` 安装同一版本。`pnpm-lock.yaml` 固定依赖解析结果；`pnpm-workspace.yaml` 声明允许执行的依赖安装脚本（esbuild）。

Check 依次运行 `pnpm install --frozen-lockfile`、`pnpm test`、`pnpm run build` 和 `pnpm run screenshots:check`。构建包含严格类型检查；截图检查确认当前版本的图片及 README 引用。Obsidian 实机验证与截图生成在本地完成。

## 发布

1. 更新 `manifest.json` 与 `package.json` 中的项目版本，完成本地验证；外观不变时重命名截图并更新引用，否则重新截图。
2. 将构建同步到 `config.local.json` 的 `defaultVaultPath` 指定的本地 vault，创建版本提交，并在该提交上添加与 manifest 完全一致的版本标签，例如 `0.1.0`。
3. 按[开发产物](DevelopmentArtifacts.md#维护)约定，归档必要的验证结论并清理已结束任务的 scratch 产物。
4. 等待用户明确要求后推送 `main` 与版本标签。
...
```sh
pnpm run package:release 0.1.0
```
