...
[PluginDocument](../../src/Persistence/PluginDocument.ts) 定义版本为 `1` 的存储格式，包括 subvault 清单和 navigation 恢复数据。领域对象只保存身份、工作路径及外观；存储解码负责验证值、清单唯一性和滚动记录。
...
- 用户发起的创建、外观修改和移除：保存成功后更新清单并发布，保存失败保留当前清单。
