...
创建面板顶部的图标与颜色按钮、底部的取消与创建按钮共用等宽填满整行的布局。外观按钮保留图形尺寸，文字随可用宽度收缩并以省略号截断，完整标签由原生延迟提示提供。文件夹选择区域包含搜索及单选树，并使用统一边框容器；树行使用自有布局与宿主语义配色，维护自身的搜索、展开和选择会话。面板覆盖文件导航期间，覆盖范围内的其他控件暂时不可交互，关闭后恢复。

共用控件及主题适配规则见[界面与主题](InterfaceAppearance.md)。容器通过独立的无障碍标签命名，控件使用原生延迟提示。
