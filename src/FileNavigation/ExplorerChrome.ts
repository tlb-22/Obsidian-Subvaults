...
export class ExplorerChrome {
  ...
  constructor(host: HTMLElement, scroll: HTMLElement, switchTo: (selection: Selection) => void, create: () => void, menu: (event: MouseEvent, button: HTMLElement, id: SubvaultId) => void) {
    ...
    this.header.className = 'sv-header';
    ...
    this.strip = host.createDiv({ cls: 'sv-switcher', attr: { role: 'toolbar' } });
  ...
  update(subvaults: readonly Subvault[], selection: Selection): void {
    ...
    this.allButton.classList.toggle('is-active', !active); this.allButton.setAttribute('aria-pressed', String(!active));
    ...
    for (const s of subvaults) {
      ...
      button.classList.toggle('is-active', active?.id === s.id); button.setAttribute('aria-pressed', String(active?.id === s.id));
