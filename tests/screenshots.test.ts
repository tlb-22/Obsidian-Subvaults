...
import { imageName, languages, scenes } from '../scripts/screenshots/Scenarios';
...
const record: CaptureRecord = {
  ...
  images: languages.flatMap(language => scenes.map(scene => ({
    ...
    observation: { language: language.obsidian, nativeNewNote: language.newNote, statusText: '72 words', hostVersion: '1.13.7', heading: scene === 'all' ? 'All' : 'Research', roots: [], externalFiles: [] },
  }))),
};
const facts: readonly ImageFact[] = record.images.map(image => ({ kind: 'present', file: image.file, sha256: image.sha256 }));

test('capture validation rejects source changes, altered images, and incorrect scene languages', () => {
  ...
  assert.equal(imageIssues('0.2.0', facts).filter(issue => issue.kind === 'missing').length, 4);
  const first = facts[0]!;
  ...
  assert.equal(captureIssues('0.1.0', 'current-source', wrongLocale, facts).filter(issue => issue.kind === 'language').length, 2);
});
